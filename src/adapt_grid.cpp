...
enum{PARTICLE,SURF,VALUE,COMPUTE,FIX,RANDOM,NSURF};
...
void AdaptGrid::process_args(int narg, char **arg)
{
  ...
  } else if (strcmp(arg[iarg],"nsurf") == 0) {
      if (iarg+3 > narg) error->all(FLERR,"Illegal adapt command");
      style = NSURF;
      int igroup = surf->find_group(arg[iarg+1]);
      if (igroup < 0)
        error->all(FLERR,"Adapt command surface group does not exist");
      sgroupbit = surf->bitmask[igroup];
      surfmax = input->inumeric(FLERR,arg[iarg+2]);
      iarg += 3;
...
bigint AdaptGrid::refine()
{
  ...
  else if (style == NSURF) refine_nsurf();
...
/* ----------------------------------------------------------------------
   refine if cell contains eligible surfs
   eligible = number of surfaces exceeds surfmax
------------------------------------------------------------------------- */

void AdaptGrid::refine_nsurf()
{
  ...
  for (int i = 0; i < rnum; i++) {
    icell = rlist[i];
    printf("icell: %i; ns: %i\n", icell, cells[icell].nsurf);
    if (cells[icell].nsurf > surfmax) rlist[n++] = icell;
  }
  rnum = n;
}
