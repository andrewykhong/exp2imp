...
/*---------------------------------------------------------------
  Convert explicit to implicit surface
-----------------------------------------------------------------*/
	// create corner values for implicit surfaces
	// initialize cvalues (corner values)
  if (dim == 2) {
  ...
  // create marching squares/cubes and invoke
  //if (dim == 2) ms = new MarchingSquares(sparta,thresh);
  //else mc = new MarchingCubes(sparta,thresh);
...
/*---------------------------------------------------------------*/
// Check how surfaces are stored (split or full)
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"origin") == 0) {
      ...
      iarg++;
		...
		// for explicit to implicit surface
    } else if (strcmp(arg[iarg],"implicit") == 0) {
			// Limitations to whern implicit surface can be made
			if (!grid->exist)
			...
			surf->global(arg[iarg]); // set now to implicit
      if (iarg+3 > narg) error->all(FLERR,"Invalid read_surf command");
			ggroup = grid->find_group(arg[iarg+1]);
